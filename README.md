...
- 2020-07-26 **New stable release: 1.7**
  ...
  **Details:**
  ...
  - QDirStat now has its own custom directory selection dialog; it no longer
    uses one of the Qt standard file dialogs:
    ...
    filesystems (the same ones that the new "Mounted Filesystems" window above
    shows).

    For each filesystem, it shows
  ...
  - The "Cross Filesystems" checkbox lets you override the global configuration
    option of the same name for this program run: QDirStat will then no longer
    stop when a mount point is found during reading a subtree, it will descend
    into that mounted filesystem and read it, too; but again, only for "real"
  ...
  - Added a workaround for wrong size sums on NTFS: Now disregarding hard links
    ...
    It appears that _ntfs-3g_ regards even the MS-DOS compatible _short_
    filename (`PROGRA~2` vs. `Program Files`) as a hard link which is of course
    utter nonsense; that means that _most_ files on an NTFS partition are
    reported as having multiple hard links, so they are reported in QDirStat as
  ...
  - <summary>
    Added a config option to ignore hard links.
     ...
     confusion; use a text editor (while QDirStat is not running).
  ...
  - There is now a new document about Btrfs free size and how different tools
    ...
    Whenever users run out of disk space on Btrfs (which happens a lot because
    of snapshots) and they try to figure out where all their disk space went,
    they are confused about different tools reporting totally different and
    inconsistent sizes, traditional Linux / Unix command line tools like `du`
    and `df` just like GUI tools like QDirStat. Hopefully, this document will
